...
function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  const htmlPositions = plainTextPositions.map(({ start, end, word }) => ({
    start: convertPlainTextIndexToHTMLIndex(start, plainText),
    end: convertPlainTextIndexToHTMLIndex(end, plainText),
    word,
  }));

  let result = htmlContent;
  let track = 0;
  for (const { start, end, word } of htmlPositions) {
    const markStart = start + track;
    const markEnd = end + track;
    result =
      result.slice(0, markStart) +`<mark>${result.slice(markStart, markEnd)}</mark>` + result.slice(markEnd);
    track += `<mark>${word}</mark>`.length - (end - start);
  }

  return result;
}

function convertPlainTextIndexToHTMLIndex(plainTextIndex, plainText) {
  const plainTextOpenTag = '<p><span>';
  const prefix = plainText.slice(0, plainTextIndex);
  return plainTextOpenTag.length + prefix.length;
}
module.exports = {
  highlightHTMLContent,
  convertPlainTextIndexToHTMLIndex,
};
